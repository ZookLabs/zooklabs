...
object Main extends IOApp {
  ...
  def run(args: List[String]): IO[ExitCode] = {
    (for {
      ...
      serverProgram       = new ServerProgram(conf, client, leagueRepository, zookRepository)
      keepAliveProgram    = new KeepAliveProgram(client)
      updateLeagueProgram = new UpdateLeagueProgram(leagueRepository)

      service <- Stream(serverProgram.run(), keepAliveProgram.run(), updateLeagueProgram.run()).parJoinUnbounded
