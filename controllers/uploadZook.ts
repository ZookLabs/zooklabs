...
export default async (context: Context) => {
    try {
        ...
        if (!contentLength || isNaN(Number(contentLength)) || Number(contentLength) > onehundredKb) {
            context.response.status = Status.BadRequest
            context.response.body = 'File Too Big'
            return
        }

        const bodyform: BodyFormData = await context.request.body({ type: 'form-data' })
        const formDataBody: FormDataBody = await bodyform.value.read({ maxFileSize: onehundredKb, maxSize: onehundredKb, bufferSize: onehundredKb })
        ...
        const zookBytesCleaned: Uint8Array<ArrayBufferLike> = trimTrailingNewlines(maybeZookFile.content)

        if (!zookBytesCleaned.slice(0, header.length).every((byte, index) => byte === header[index])) {
            context.response.status = Status.BadRequest
        ...
        const owners: Array<any> = Array.isArray(ownersRaw) ? ownersRaw : [ownersRaw];
        ...
        const details: Array<any> = Array.isArray(detailsRaw) ? detailsRaw : [detailsRaw];

        const getPhysicalDetail = (name: string): number => {
            return Number(details.filter((detail) => detail["@name"] === name)[0]["@data"].trim());
        }
        ...
        const zookId = await persistZook(zookContainer, (zookId: number) => {
            ...
            return writeZookAndImage(zookId.toString(), zookName, zookBytesCleaned, pngBytes)
        });

        await sendDiscordWebhook(zookContainer.zook, authUser, zookId, pngBytes, discordWebhook);
...
async function sendDiscordWebhook(
    ...
    const embed: Embed = {
        ...
        color: 16725286,
        description: user?.username ? `__Uploaded By__:\n**${user.username}**` : undefined,
